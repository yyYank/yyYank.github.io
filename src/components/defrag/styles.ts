...
.dfg{
  ...
  --deeper:#131620; --onlamp:#141621; --placeholder:#454B5A; --dim:#525869;
  --disabled:#4E5464; --meta:#5D6474; --icon:#6B7284; --hint:#565C6C;
  --drop-bg:#1D1B18; --active-bg:#20242F; --active-border:#39404F;
  --swipe-bg:#2A1A1D; --swipe-text:#8C5A60; --swipe-armed-bg:#3A1F24; --swipe-armed-text:#E08088;
  --quote:#4E647A; --quote-text:#98A0B0; --sim-text:#A8AEBC; --warn:#C2757A;
  --note-text:#23262E; --input-bg:#0E1016;
  --note-plain:#EDE9DE; --note-plain-edge:#BCB49F;
  --note-amber:#F5D68B; --note-amber-edge:#C99A2E;
  --note-rose:#F2B9BC; --note-rose-edge:#C46E76;
  --note-teal:#A9DBD2; --note-teal-edge:#489C93;
  --note-indigo:#BCC6F0; --note-indigo-edge:#6E7BC9;
  --note-olive:#D2DFA4; --note-olive-edge:#8DA34F;
  position:fixed;top:64px;left:0;right:0;bottom:0;display:flex;flex-direction:column;
...
.dfg-drawer{position:absolute;inset:0;z-index:60;background:var(--deeper);display:flex;flex-direction:column;
...
.dfg-ta::placeholder{color:var(--placeholder)}
...
.dfg-recent{flex:1;min-width:0;font-size:12px;line-height:1.5;color:var(--dim)}
.dfg-recent span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dfg-send{flex:0 0 auto;width:66px;height:66px;border-radius:50%;border:0;background:var(--lamp);color:var(--onlamp);
...
.dfg-send:disabled{background:var(--raised);color:var(--disabled);box-shadow:none}
...
.dfg-tlrow em{font-style:normal;font-size:10.5px;color:var(--meta)}
.dfg-tlempty{padding:28px 0;text-align:center;color:var(--disabled);font-size:12.5px}
...
.dfg-rootrow[data-drop="1"]{border-style:solid;border-color:var(--lamp);background:var(--drop-bg)}
.dfg-rootrow em{font-style:normal;font-size:10.5px;color:var(--meta);font-family:ui-monospace,monospace}
...
.dfg-folderbox[data-on="1"]{background:var(--active-bg);border-color:var(--active-border);box-shadow:inset 3px 0 0 var(--lamp)}
.dfg-folderbox[data-drop="1"]{border-color:var(--lamp);box-shadow:0 0 0 1px var(--lamp) inset;background:var(--drop-bg)}
...
.dfg-node em{font-style:normal;font-size:10.5px;color:var(--meta);font-family:ui-monospace,monospace;flex:0 0 auto}
.dfg-caret{appearance:none;flex:0 0 auto;width:46px;align-self:stretch;background:transparent;
  border:0;color:var(--icon);display:flex;align-items:center;justify-content:center;
...
.dfg-folder{flex:0 0 auto;color:var(--icon)}
.dfg-folderbox[data-on="1"] .dfg-folder{color:var(--lamp)}
.dfg-act{appearance:none;border:0;background:transparent;color:var(--disabled);width:38px;flex:0 0 auto;font-size:15px;
...
.dfg-swipebg{position:absolute;inset:0;border-radius:9px;background:var(--swipe-bg);display:flex;align-items:center;
  justify-content:flex-end;padding-right:18px;font-size:12.5px;font-weight:700;letter-spacing:.1em;color:var(--swipe-text)}
.dfg-swipebg[data-armed="1"]{background:var(--swipe-armed-bg);color:var(--swipe-armed-text)}
...
.dfg-leaf em{font-style:normal;font-size:10px;color:var(--meta);flex:0 0 auto;font-family:ui-monospace,monospace}
.dfg-pip{width:5px;height:5px;border-radius:50%;flex:0 0 auto}
.dfg-stackmark{flex:0 0 auto;width:13px;height:13px;border:1px solid var(--meta);border-radius:3px;position:relative}
.dfg-stackmark::after{content:"";position:absolute;left:2px;right:-3px;top:3px;bottom:-3px;border:1px solid var(--meta);
  border-radius:3px;background:var(--panel)}
.dfg-empty{padding:44px 24px;text-align:center;color:var(--disabled);font-size:13.5px;line-height:2}
...
.dfg-quick button{flex:0 0 auto;width:46px;height:46px;border-radius:12px;border:0;background:var(--lamp);
  color:var(--onlamp);font-family:inherit;font-size:14px;font-weight:700;cursor:pointer}
.dfg-quick button:disabled{background:var(--raised);color:var(--disabled)}

.dfg-echo{flex:0 0 auto;border-left:2px solid var(--quote);padding:2px 0 2px 11px;margin-bottom:4px;
  animation:dfgfade .3s}
.dfg-echohead{display:block;font-size:10px;letter-spacing:.12em;color:var(--quote);margin-bottom:6px}
.dfg-echo button{display:block;width:100%;text-align:left;appearance:none;border:0;background:transparent;
  color:var(--quote-text);font-family:inherit;font-size:12.5px;line-height:1.55;padding:3px 0;cursor:pointer;
  overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.dfg-note{position:absolute;width:46%;min-height:112px;border-radius:3px;padding:11px 11px 24px;
  border:1px solid rgba(0,0,0,.16);border-top:4px solid var(--edge);touch-action:pan-y;color:var(--note-text);
...
.dfg-note q{quotes:none;flex:1;font-size:12.5px;line-height:1.62;color:var(--note-text);overflow:hidden;
...
.dfg-wallhint{font-size:11.5px;color:var(--hint);line-height:1.7;padding:10px 14px max(24px,env(safe-area-inset-bottom))}
...
.dfg-scrollx::-webkit-scrollbar-thumb{background:var(--line);border-radius:2px}
.dfg-vizfoot{font-size:10.5px;color:var(--hint);letter-spacing:.08em;padding:10px 2px 0}
...
.dfg-newname{flex:1;min-width:0;background:var(--input-bg);border:1px solid var(--lamp);border-radius:5px;color:var(--ink);
...
.dfg-btn[data-key="1"]{background:var(--lamp);border-color:var(--lamp);color:var(--onlamp)}
.dfg-btn[data-quiet="1"]{background:transparent;border-color:transparent;color:var(--muted);padding:12px 10px}
.dfg-btn[data-warn="1"]{color:var(--warn);border-color:transparent;background:transparent}
...
.dfg-label{font-size:10.5px;letter-spacing:.14em;color:var(--hint);margin:20px 0 9px}
...
.dfg-hint{font-size:11.5px;color:var(--hint);line-height:1.7;margin-top:10px}
.dfg-sim{appearance:none;font-family:inherit;text-align:left;cursor:pointer;width:100%;
  background:var(--panel);border:1px solid var(--line);border-left:2px solid var(--quote);border-radius:8px;
  padding:11px 12px;margin-bottom:8px;font-size:13px;line-height:1.6;color:var(--sim-text);display:-webkit-box;
...
.dfg-mini{appearance:none;border:0;background:transparent;color:var(--meta);font-family:inherit;font-size:16px;
...
.dfg-chip[data-on="1"]{background:var(--lamp);border-color:var(--lamp);color:var(--onlamp)}
...
.dfg-srmain em{font-style:normal;font-size:10.5px;color:var(--meta);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.dfg-tweeterr{margin-top:10px;font-size:12.5px;color:var(--warn)}
...
.dfg-commentmeta{display:block;font-size:10.5px;color:var(--meta);margin-top:5px}
